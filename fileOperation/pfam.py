...
def step(lines, p_filter, s_length, output_type):
    ...
    for line in lines:
        ...
        if len(items) > 3 and float(items[2]) > p_filter:
            if items[1] not in pairs[items[0]]:
                if items[0] not in genes.keys():
                    return "Error: require length of gene " + items[0]
                if items[1] not in genes.keys():
                    return "Error: require length of gene " + items[1]
    ...
    if output_type == 'text':
        # write to output
        output = ''
        for item in sort_humanly(d_out.keys()):
            if item not in removed:
                result_line = ''
                length = ''
                if len(d_out[item]) > 1:
                    for j in range(len(d_out[item])):
                        result_line += d_out[item][j]
                        r = d_out[item][j]
                        if r in genes.keys():
                            length += genes[r]
                        if j < len(d_out[item]) - 1:
                            result_line += ', '
                            length += ', '
                    output += item + '\t' + result_line + '\t' + length + '\n'
    else:
...
def pfam_to_oneline(lines, file, find_type, file_output_type, file_name):
    # pfam to one line
    ...
    for line in lines:
        line = line.strip('\n')
        items = line.split('\t')
        if len(items) > 12 and items[3] == find_type:
            temp_key = items[0] + '/' + items[4]
            d_pfam[temp_key].append(Pfam(items[0], items[3], items[4], items[5], items[8], items[11], items[12]))
        elif len(items) > 10 and items[3] == find_type:
            temp_key = items[0] + '/' + items[4]
            d_pfam[temp_key].append(Pfam(items[0], items[3], items[4], items[5], items[8], '', ''))
    ...
    # find pfam
    result_filename = ''
    if file_output_type == 'file':
        result_filename = out_file_name(file_name)
                ...
                if len(d_file[file_key]) > 1:
                    for n in range(len(d_file[file_key])):
                        ...
                        if n < len(d_file[file_key]) - 1:
                            ...
                            pfam_ipr_line += '; '
                            pfam_domain2_line += '; '
                    f.write(file_key + '\t' + blast_relate + '\t' + blast_length + '\t' + find_type + '\t' +
                            pfam_pf_line + '\t' + pfam_domain1_line + '\t' +
                            pfam_ipr_line + '\t' + pfam_domain2_line + '\n')
    elif file_output_type == 'text':
        for file_key in sort_humanly(d_file.keys()):
            blast_relate = ''
            blast_length = ''
            pfam_pf_line = ''
            ...
            if len(d_file[file_key]) > 1:
                for n in range(len(d_file[file_key])):
                    ...
                    if n < len(d_file[file_key]) - 1:
                        ...
                        pfam_ipr_line += '; '
                        pfam_domain2_line += '; '
                result_filename += file_key + '\t' + blast_relate + '\t' + blast_length + '\t' + find_type + '\t'\
                                   + pfam_pf_line + '\t' + pfam_domain1_line + '\t' \
                                   + pfam_ipr_line + '\t' + pfam_domain2_line + '\n'
...
def out_file_name(ofile):
    # output file name
    name = ofile.split('.')
    if len(name) > 1:
        n = ".".join(name[:-1])
        ext = name[-1]
        filename = n + "_" + time.strftime('%Y%m%d%H%M%S') + '.' + ext
    else:
        filename = ofile + "_" + time.strftime('%Y%m%d%H%M%S')
    return filename
